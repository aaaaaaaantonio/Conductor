...
### Parked from final whole-branch review (2026-09-11, commits aa9a78f..eaa44a8) — real, not fixed, not merge blockers

- **No UI to create reference data, teams, agents, or tests.** Spec §4.1/§4.3/§5.5/§5.8 all assume data is added through the UI without a deploy. What shipped is read-only (flags/fields render as static tables); the only way to populate any of it today is `curl`/direct API calls. Needs its own task(s): reference-data admin panel, team↔stand link editor, agent/test constructor UI.
- **"Проверить" (check) button and Jenkins build-ID field — spec §3.1.** The plan's own File Structure comment for `routers/jobs.py` says "job list, SSE stream, **check/restart**", but no task implements "Проверить" (only "Перезапуск" was explicitly deferred above). Needs scoping.
- **`asyncio.Queue` + background worker pool — spec §2.** Implementation uses FastAPI `BackgroundTasks` with unbounded concurrency instead: N simultaneous launches spawn N subprocesses with no serialization or cap, and `status="queued"` is vestigial for local jobs (queued→running is instantaneous). Fine for MVP traffic; matters once concurrent "долгие тесты" (spec §1) are common.
- **`Job.updated_at` is set once at insert and never bumped on status transitions** (`runner.py`'s queued→running→terminal writes, `main.py`'s startup recovery, the agent-test launch endpoint). Misleads any future "recent jobs" view. Fix is small — a shared `_set_status(job, status)` helper that also stamps `updated_at`, used at every write site.
- **No nav / no `/` route.** `python_tab.html` and `agent_testing.html` aren't linked to each other; users must type `/python` or `/agent-testing` by hand, and `GET /` 404s. A three-line nav in `base.html` plus a `/` redirect closes this.
- **`LOG_DIR = Path("job_logs")` duplicated in two routers, both relative to process CWD.** Logs land wherever uvicorn was started from. Move to one shared location, anchor to an absolute path or env var.
- **Live command preview (spec §5.8)** on the agent-test card — mentioned in Task 12's brief Interfaces line but never built (brief's own Step 3 code omits it too). Small addition once scoped: reuse `command_builder.build_command` client-side-equivalent or a preview endpoint.
- Smaller items noted by the same review, not yet actioned: `.gitignore` doesn't cover `venv/`, `.venv/`, `test_runner.db`, `job_logs/`; `references.py`'s create endpoint accepts the full `ReferenceItem` table model as request body (client could set `is_active`/`sort_order` directly) instead of a narrow `ReferenceItemCreate` schema; `runner.py` is inconsistent about raising vs. swallowing on different subprocess-failure paths; soft-deleting a team silently orphans its `Agent` rows from the tree view; `@app.on_event("startup")` is deprecated (FastAPI wants `lifespan`); the in-process `EventBroadcaster` only delivers SSE events to the uvicorn worker that ran the job — breaks under `--workers >1`.
